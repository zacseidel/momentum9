...
1.  **The "Tesla" Model (Joby):** You believe that controlling the whole stack (making the plane AND the app) yields the highest margins long-term, even if it burns more cash now.
2.  **The "Automotive" Model (Archer):** You believe that aviation is hard, so you should let car companies (Stellantis) build the planes and airlines (United) fly them to reduce risk.
3.  **The "Pick & Shovel" Proxy:** The ETF **ARKX** (ARK Space Exploration & Innovation) holds both Joby and Archer, serving as a sector proxy if you prefer not to pick a single winner.
